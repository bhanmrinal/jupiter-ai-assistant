...
class BlogScraper(BaseScraper):
    ...
    def extract_content(self, soup: BeautifulSoup, url: str) -> dict[str, Any]:
        ...
        date_selectors = [".post-date", ".entry-date", ".published", "time[datetime]", ".date"]
    ...
    def _extract_tags(self, soup: BeautifulSoup) -> list[str]:
        ...
        tag_selectors = [".post-tags a", ".entry-tags a", ".tags a", ".tag-links a"]
    ...
    def _extract_heading_qa(self, content: str, content_data: dict[str, Any]):
        ...
        for i, line in enumerate(lines):
            ...
            if len(line) > 10 and (
                "?" in line
                or any(word in line.lower() for word in ["how", "what", "why", "when", "where"])
            ):
